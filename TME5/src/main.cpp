...
int main()
{
	...
	pr::Pool pool(height); // Un job par ligne
	pool.start(32);		   // ou le nombre de threads que vous voulez utiliser

	pr::Barrier barrier(height);

	for (int y = 0; y < height; ++y)
	{
		pr::Job *job = new LinePixelJob(scene, screen, lights, pixels, y, width, barrier);
		pool.submit(job);
	}

	barrier.waitFor(); // Attendre que toutes les lignes soient traitées
	pool.stop();

	// On attend que tous les pixels soient traités
	barrier.waitFor();
	pool.stop();
